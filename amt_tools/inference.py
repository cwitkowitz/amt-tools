...
def run_online(track_data, model, estimator=None):
    ...
    features = tools.try_unpack_dict(track_data, tools.KEY_FEATS)
    times = tools.try_unpack_dict(track_data, tools.KEY_TIMES)
    ...
    for i in range(num_frame_groups):
        ...
        new_predictions = tools.dict_squeeze(tools.dict_to_array(model.run_on_batch(batch)), dim=0)
    ...
    predictions = tools.dict_squeeze(tools.dict_to_array(predictions), dim=0)
